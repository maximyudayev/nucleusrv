...
//   // AMO stall logic
//   // landh: amo_stall should be asserted when amo_state is in read phase or write phase
//   // val amo_stall = (amo_state === s_READ) || (amo_state === s_WRITE)
//   val amo_stall = (amo_state =/= s_IDLE)
...
//   // landh: add amo_stall to hazard unit (in ID module) to stall pipeline during AMO operations
//   ID.mem_stall_req := amo_stall
...
//   // IF-ID Atomic signals
//   id_reg_isAMO := ID.isAMO
//   id_reg_isLR  := ID.isLR
//   id_reg_isSC  := ID.isSC
//   id_reg_amoOp := ID.amoOp

//   // Stall (freeze) IF/ID registers if hazard unit signall is called
//   when(!ID.hdu_id_ex_write) {
//     id_reg_pc := id_reg_pc
...
//     id_reg_isAMO := id_reg_isAMO
//     id_reg_isLR  := id_reg_isLR
//     id_reg_isSC  := id_reg_isSC
//     id_reg_amoOp := id_reg_amoOp
//     if(F) {
...
//   // landh: For AMO instructions, the ALU result (ex_reg_result) must be the 
//   // address, which is rs1 (id_reg_rd1).
//   // For all other instructions, it's the normal ALU calculation.
//   ex_reg_result := Mux(id_reg_isAMO, id_reg_rd1, EX.ALUresult)
//   // ex_reg_result := EX.ALUresult

//   // landh: Stall ID/EX registers if hazard unit signal is called
//   when (!ID.hdu_ex_mem_write) {
...
//     ex_reg_isAMO := ex_reg_isAMO
//     ex_reg_isLR := ex_reg_isLR
//     ex_reg_isSC := ex_reg_isSC
//     ex_reg_amoOp := ex_reg_amoOp
//     if(F) {
...
//     is(s_IDLE) {  // IDLE
//       when(amo_is_active) {
//         amo_state := s_READ  // Start read phase
//       }
//     }
//     is(s_READ) {  // READING
...
//         amo_state := s_WRITE  // Move to write phase
//       }
//     }
//     is(s_WRITE) {  // WRITING
...
//       }
//     }
//     // is(s_RETIRED) {  // RETIRED
//     //   // After retiring, go back to IDLE
//     //   amo_state := s_IDLE
//     // }
//   }
...
//   amoALU.io.memData := Mux(amo_state === s_READ && MEM.io.dccmRsp.valid, 
//                             MEM.io.readData, 
//                             amo_read_data)
...
//                        (ex_reg_isAMO && (amo_state === s_IDLE || amo_state === s_READ)) || 
//                        ex_reg_isLR
...
//                         (ex_reg_isAMO && amo_state === s_WRITE) || 
...
//   MEM.io.writeData := Mux(ex_reg_isAMO && amo_state === s_WRITE, 
//                          amo_modified_data,
//                          ex_reg_wd)
...
//   // Stall (freeze) EX/MEM registers if hazard unit signall is called
//   when (!ID.hdu_mem_wb_write) {
//     mem_reg_rd := mem_reg_rd
//     mem_reg_ins := mem_reg_ins
//     mem_reg_result := mem_reg_result
//     mem_reg_branch := mem_reg_branch
...
//     mem_reg_isAMO := mem_reg_isAMO
//     mem_reg_isLR :=  mem_reg_isLR
//     mem_reg_isSC :=  mem_reg_isSC
//     if(F) {
//        mem_reg_f_read.get := mem_reg_f_read.get
//        mem_reg_f_except.get := mem_reg_f_except.get
//        mem_reg_is_f.get := mem_reg_is_f.get
//     }
//   }
